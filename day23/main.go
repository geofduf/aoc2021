...
// h represents the hallway.
// mobile represents the number of misplaced amphipods in each room.
// empty represents the number of empty spots in each room.
// cost represents the current cost.
// remaining represents the total number of misplaced amphipods (rooms & hallway).
type State struct {
	h         [11]uint8
	mobile    [4]uint8
	empty     [4]uint8
...
func main() {
	...
	if len(chars) == 8 {
		p1, t1 := processInput(chars, experimental)
		fmt.Printf("Part1: found cost of %d in %s\n", p1, t1)
	} else {
		var part1 []byte
		part1 = append(part1, chars[:4]...)
		p1, t1 := processInput(append(part1, chars[12:16]...), experimental)
		fmt.Printf("Part1: found cost of %d in %s\n", p1, t1)
		p2, t2 := processInput(chars, experimental)
...
func processInput(chars []byte, experimental bool) (uint32, time.Duration) {
	...
	for i, v := range chars {
		if i%4 == 0 {
			rooms = append(rooms, [4]uint8{})
		}
		rooms[len(rooms)-1][i%4] = v - 65
	}
	...
	for r := len(rooms) - 1; r >= 0; r-- {
		for c, v := range rooms[r] {
...
STACK:
	for len(stack) > 0 {
		...
		// Process state. The general pattern is to perform all possible hallway->room and room->room
		// moves in-place (swapping not implemented) then start stacking new states for room-hallway
		// moves.

		done := false

		// Restart the search until we can't perform any in-place moves (a move might enable other ones).
		for !done {
